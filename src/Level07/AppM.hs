...
instance Monad (AppM e) where
  ...
  (>>=) (AppM ea) fa = AppM $ (\env -> ea env 
    >>= (\ea' -> case ea' of
      Left e -> pure $ Left e
      Right a -> runAppM (fa a) env)) 

instance MonadError e (AppM e) where
  throwError :: e -> AppM e a
  throwError = AppM . pure . pure . Left 

  catchError :: AppM e a -> (e -> AppM e a) -> AppM e a
  catchError (AppM ea) fe = AppM $ (\env -> ea env
    >>= (\ea' -> case ea' of 
      Left e -> runAppM (fe e) env
