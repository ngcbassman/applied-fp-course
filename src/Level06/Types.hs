...
partialConfDecoder = PartialConf
  <$> ((Last . fmap Port) <$> D.atKeyOptional "port" D.integral)
  <*> ((Last . fmap DBFilePath) <$> D.atKeyOptional "dbFilePath" D.string)

-- atLast :: Text -> Decoder f a -> b -> Decoder f (Last b)
-- atLast field decoder p = (Last . fmap p) <$> D.atKeyOptional field decoder

partialConfEncoder :: Monad f => Encoder f PartialConf
partialConfEncoder = E.mapLikeObj $ \pc ->
  E.atKey' "port" encodePort (pcPort pc) .
  E.atKey' "dbFilePath" encodeDBFilePath (pcDBFilePath pc)
  
encodePort :: Monad f => Encoder f (Last Port)
encodePort = (\a -> case getLast a of
  Nothing -> undefined
  (Just p) -> getPort p
  ) >$< E.integral

encodeDBFilePath :: Monad f => Encoder f (Last DBFilePath)
encodeDBFilePath = (\a -> case getLast a of
  Nothing -> undefined
  (Just fp) -> getDBFilePath fp
  ) >$< E.string
